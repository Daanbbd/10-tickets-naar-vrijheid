...
func _ready() -> void:
	...
	root.offset_left = 4; root.offset_right = -4
	root.offset_top = 4; root.offset_bottom = -4
	root.add_theme_constant_override("separation", 4)
	add_child(root)

	var head := UiKit.label("WIE BEN JIJ VANDAAG", UiKit.FS_BODY, UiKit.BLUEBIRD_BRIGHT)
	head.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	root.add_child(head)

	var sub := UiKit.label("Wisselen kan niet. Zo werkt dat hier niet.",
		UiKit.FS_SMALL, UiKit.GRIJS)
	sub.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	sub.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	root.add_child(sub)

	# Portret: vijf kaarten naast elkaar passen niet op 192 px. Een raster van
	# drie kolommen wel, en de portretten blijven daarin groot genoeg om iemand
	# aan zijn gezicht te herkennen.
...
func _make_card(c: CharacterDef) -> PanelContainer:
	...
	p.custom_minimum_size = Vector2(59, 74)
	_cards.append(p)
	...
	portrait.custom_minimum_size = Vector2(32, 40)
	...
	# de rol staat in het detailpaneel; op een kaart van 59 px zou hij over
	# drie regels breken en de kaarten ongelijk hoog maken
